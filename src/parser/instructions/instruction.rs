...
impl Instruction {
    pub fn from_command(command: String, args: ArgList, line_num: usize) -> Result<Instruction> {
        ...
        match command.as_str() {
            "branch" => {
                ...
                let branch_args = args.named_args.iter().skip(1).try_fold(
                    BranchArgs::default(),
                    |ba, arg| match arg.name.as_str() {
                        "new_root" => Result::Ok(ba.with_new_root()),
                        _ => bail!(ErrorKind::InvalidArgument(command.clone(), arg.name.clone(), line_num))
                    }
                )?;
            ...
            "merge" => {
                ...
                Ok(Instruction::MERGE(
                    ...
                    args.plain_args.iter().skip(1).map(
                        |a| a.clone()
                    ).collect()
                ))
            },
    ...
    pub fn from_line(line: &str, line_num: usize) -> Result<Instruction> {
        lazy_static! {
            static ref COMMAND_RE: Regex = Regex::new(r"^(\s*[a-z]+)\((?:([^,\n]+),)*([^,\n]+)\)\s*$").unwrap();
        }
